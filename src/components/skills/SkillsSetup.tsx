...
const platforms = [
  ...
  {
    id: "copilot", name: "Microsoft Copilot", note: "This route uses Copilot Studio, not a universal setting in everyday Copilot chat. Ask the person who manages your Microsoft tools to help configure an agent.",
    steps: ["In Copilot Studio, have your agent administrator configure a custom MCP connection using the address below (Streamable HTTP; no authentication).", "On the agent’s Tools page, choose Add a tool → Model Context Protocol and select the configured connection. Add and configure it, keeping skill discovery and local search tools enabled.", "Test the prompt below in the agent, then publish or share the agent with your team when ready."],
...
export default function SkillsSetup() {
  return <section id="setup" className="container px-4 pb-16 scroll-mt-24">
    <div className="max-w-5xl mx-auto rounded-2xl border bg-card p-6 md:p-8">
      ...
      <h3 className="font-heading text-lg font-semibold mb-2">What is MCP?</h3>
      <p className="text-sm text-muted-foreground leading-relaxed mb-6">MCP stands for Model Context Protocol. Think of it as a connection between your AI assistant and an information service. Our connection supplies playbooks and local information; your AI uses them to help you plan and create. It is read-only: it cannot send emails, register for events or change business records. You review the output and decide what happens next.</p>
      <p className="font-medium mb-3">1. Choose your AI tool</p>
      <Tabs defaultValue="claude">
        ...
        {platforms.map((platform) => <TabsContent key={platform.id} value={platform.id} className="pt-4">
          ...
          <ol className="list-decimal pl-5 space-y-3 text-sm leading-relaxed">{platform.steps.map((step) => <li key={step}>{step}</li>)}</ol>
      ...
      <div className="mt-6"><p className="font-medium mb-3">2. Copy this connection address when prompted</p><McpUrlBox url="https://startuporillia.ca/mcp" /></div>
      <div className="mt-6"><p className="font-medium mb-3">3. Start a conversation</p><blockquote className="rounded-lg bg-secondary p-4 text-sm leading-relaxed">Use Startup Orillia to find and read the right skill for this: I run a bakery in Orillia and January weekday sales are slow. Help me compare three campaign ideas, using relevant local businesses where useful.</blockquote><p className="text-sm text-muted-foreground mt-3">Then try: “Build idea #2. Give me the offer, outreach drafts, a 30-day plan and a way to measure results.” If your AI has not used the connection, ask it to list Startup Orillia’s skills and read the relevant one first.</p></div>
      <p className="text-sm text-muted-foreground mt-5">The connection retrieves playbooks when needed; it does not install them permanently in your AI tool. Local business information currently comes from a monthly snapshot of publicly available <a href="https://business.orillia.com/" target="_blank" rel="noreferrer" className="underline">Orillia &amp; District Chamber of Commerce information</a>. Check original listings for current details.</p>
      <details className="border-t mt-6 pt-5"><summary className="cursor-pointer font-medium">No connection option? Use a skill without MCP</summary><div className="text-sm text-muted-foreground space-y-3 mt-4"><p>Open a skill in the catalog, copy its full instructions, and paste them into your AI conversation with your business problem. Ask your AI to follow the playbook. Without a local-data connection, provide any business or event links you want it to use.</p><p>You can also download individual skill ZIPs, or the whole collection below. In Claude’s supported Skills workflow, use Customize → Skills → + Create skill → Upload a skill. The collection ZIP contains separate folders; it is not a universal one-click installer.</p><p>For coding agents, the Skills CLI can install the collection from our existing repository:</p><pre className="overflow-x-auto bg-secondary rounded-lg p-3">npx skills add startuporillia/startuporillia --skill '*'</pre><a href="https://github.com/vercel-labs/skills" className="underline" target="_blank" rel="noreferrer">Skills CLI instructions</a></div></details>
    </div>
  </section>;
}
